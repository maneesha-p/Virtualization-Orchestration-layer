...
def main():
    ...
    for line in f:
        ...
        global wholepath
        global hostname
        wholepath.update({id1 : line.rstrip()})
        str4=line.split(':')[-1]
        str1=line.split('/')[-1]
        imagepaths.update({id1 : str4.rstrip()})
    ...
    for line in f:
        ...
        pmid_conn_mapping.update({id1 : conn})
        id1+=1
    f.close()
...
def list_vms(pmid):
    ...
    pmid=int(pmid)
    out=[{'vmids':values} for key,values in pm_vm_mapping.iteritems()]
    return {'vmids':out}
...
def vm_query():
    ...
    for key,values in vm_type_mapping.iteritems():
        if key==x:
            in_type=values
            break
